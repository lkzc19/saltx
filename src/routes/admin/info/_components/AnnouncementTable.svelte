<script lang="ts">
	...
	const columns = [
		{ header: '封面', width: '7rem', headerClass: 'text-center' },
		{ header: '标题' },
		{ header: '分类' },
		{ header: '推荐', width: '5rem', headerClass: 'text-center' },
...
>
	{#snippet header({ col })}
		{#if col.header === ''}
			<button
				onclick={onadd}
				class="flex h-6 w-6 items-center justify-center rounded bg-cf text-sm text-white transition-opacity hover:opacity-90"
				aria-label="新增动向"
			>+</button>
		{:else}
			{col.header}
	...
	{#snippet row(item)}
		...
		<td class="cursor-pointer border-b border-r border-border-primary px-4 align-middle text-text-primary" onclick={() => onselect(item)}>
			<span class="line-clamp-1 text-xs text-text-disabled">{item.category}</span>
		</td>
		<td class="border-b border-r border-border-primary px-4 align-middle text-center">
			<button
				type="button"
				role="switch"
				aria-checked={item.is_recommended === 'true'}
				aria-label="切换推荐状态"
				onclick={(e) => {
					e.stopPropagation();
					ontogglerecommended(item.id);
				}}
				class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors"
				class:bg-cf={item.is_recommended === 'true'}
				class:bg-border-primary={item.is_recommended !== 'true'}
			>
				<span
					class="inline-block h-3.5 w-3.5 rounded-full bg-white transition-transform"
					class:translate-x-[18px]={item.is_recommended === 'true'}
					class:translate-x-[3px]={item.is_recommended !== 'true'}
				></span>
			</button>
		</td>
